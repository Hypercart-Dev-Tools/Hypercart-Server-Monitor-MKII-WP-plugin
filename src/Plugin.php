...
class Plugin {
	...
	private function register_hooks() {
		...
		// Register cron hook.
		add_action( 'wp_server_monitor_run', array( $this, 'run_monitoring' ) );

		// Add settings link on plugins page.
		add_filter(
			'plugin_action_links_' . WP_SERVER_MONITOR_PLUGIN_BASENAME,
	...
	public function add_settings_link( $links ) {
		$settings_link = sprintf(
			'<a href="%s">%s</a>',
			esc_url( admin_url( 'tools.php?page=wp-server-monitor' ) ),
			esc_html__( 'Settings', 'wp-server-monitor' )
		);
	...
	public function register_admin_menu() {
		add_management_page(
			__( 'Server Performance Monitor', 'wp-server-monitor' ),
			__( 'Server Monitor', 'wp-server-monitor' ),
			'manage_options',
			'wp-server-monitor',
	...
	public function render_admin_page() {
		echo '<div class="wrap">';
		echo '<h1>' . esc_html__( 'Server Performance Monitor', 'wp-server-monitor' ) . '</h1>';
		echo '<p>' . esc_html__( 'Admin UI coming in Phase 4...', 'wp-server-monitor' ) . '</p>';
		echo '</div>';
	}
	...
	public static function activate() {
		...
		\Hypercart_Logger::info(
			...
			array(
				'cron_scheduled' => wp_next_scheduled( 'wp_server_monitor_run' ) !== false,
			)
		);
	}
	...
	public static function deactivate() {
		...
		$timestamp = wp_next_scheduled( 'wp_server_monitor_run' );
		if ( $timestamp ) {
			wp_unschedule_event( $timestamp, 'wp_server_monitor_run' );
		}
		...
		\Hypercart_Logger::info(
			...
			array(
				'cron_unscheduled' => wp_next_scheduled( 'wp_server_monitor_run' ) === false,
			)
		);
	}
}
